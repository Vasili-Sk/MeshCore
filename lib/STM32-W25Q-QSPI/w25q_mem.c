 ...
 *******************************************
 * @file    w25q_mem.c
 * @author  Dmitriy Semenov / Crazy_Geeks, modified by VasiliSk
 * @version 0.2b
 * @date    10-2025
...
#ifndef W25Q_DATA_LINES
#define W25Q_DATA_LINES 1
#endif

#if W25Q_DATA_LINES==1
#define W25Q_ADDRESS_MODE QSPI_ADDRESS_1_LINE
#define W25Q_DATA_RMODE   QSPI_DATA_1_LINE
#define W25Q_DATA_READ    W25Q_FAST_READ
#define W25Q_DATA_WRITE   W25Q_PAGE_PROGRAM
#define W25Q_DATA_WMODE   QSPI_DATA_1_LINE
#define W25Q_READ_DUMMY   8
#endif

#if W25Q_DATA_LINES==2
#define W25Q_ADDRESS_MODE QSPI_ADDRESS_2_LINES
#define W25Q_DATA_RMODE   QSPI_DATA_2_LINES
#define W25Q_DATA_READ    W25Q_FAST_READ_DUAL_IO
#define W25Q_DATA_WRITE   W25Q_PAGE_PROGRAM
#define W25Q_DATA_WMODE   QSPI_DATA_1_LINE
#define W25Q_READ_DUMMY   4 //based in DC bit in read configuration register
#endif

#if W25Q_DATA_LINES==4
#define W25Q_ADDRESS_MODE QSPI_ADDRESS_4_LINES
#define W25Q_DATA_RMODE   QSPI_DATA_4_LINES
#define W25Q_DATA_READ    W25Q_FAST_READ_QUAD_IO
#define W25Q_DATA_WRITE   W25Q_PAGE_PROGRAM_QUAD_INP
#define W25Q_DATA_WMODE   QSPI_DATA_4_LINES
#define W25Q_READ_DUMMY   6
#endif
/**
...
#if W25Q_FLASH_SIZE > 128 // if 4-byte mode
...
W25Q_STATE W25Q_ReadRaw(uint8_t *buf, uint16_t data_len, uint32_t rawAddr) {
	...
	QSPI_CommandTypeDef com;
	com.InstructionMode = QSPI_INSTRUCTION_1_LINE; // QSPI_INSTRUCTION_...
#if W25Q_FLASH_SIZE > 128U
	com.Instruction = W25Q_DATA_READ; //W25Q_FAST_READ_QUAD_IO_4B;	 // Command
	com.AddressSize = QSPI_ADDRESS_32_BITS;
#else
	com.Instruction = W25Q_DATA_READ;	 // Command
	com.AddressSize = QSPI_ADDRESS_24_BITS;
#endif
	
	com.AddressMode = W25Q_ADDRESS_MODE;
	com.Address = rawAddr;
	...
	com.DummyCycles = W25Q_READ_DUMMY;
	com.DataMode = W25Q_DATA_RMODE;
	com.NbData = data_len;
...
W25Q_STATE W25Q_ProgramRaw(uint8_t *buf, uint16_t data_len, uint32_t rawAddr) {
	...
	com.InstructionMode = QSPI_INSTRUCTION_1_LINE; // QSPI_INSTRUCTION_...
#if W25Q_FLASH_SIZE > 128U
	com.Instruction = W25Q_DATA_WRITE;//W25Q_PAGE_PROGRAM_QUAD_INP_4B;	 // Command
	com.AddressSize = QSPI_ADDRESS_32_BITS;
#else
	com.Instruction = W25Q_DATA_WRITE;	 // Command
	com.AddressSize = QSPI_ADDRESS_24_BITS;
#endif
	com.AddressMode = QSPI_ADDRESS_1_LINE;
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_ErasePage(uint32_t PageIndex) {
	...
	uint32_t rawAddr = PageIndex * W25Q_PAGE_SIZE;
	...
	com.InstructionMode = QSPI_INSTRUCTION_1_LINE; // QSPI_INSTRUCTION_...
	com.Instruction = W25Q_PAGE_ERASE;	 // Command
...
W25Q_STATE W25Q_EraseSector(uint32_t SectAddr) {
	...
	uint32_t rawAddr = SectAddr * W25Q_SECTOR_SIZE * 1024U;
...
W25Q_STATE W25Q_EraseBlock(uint32_t BlockAddr, uint8_t size) {
	...
	uint32_t rawAddr = BlockAddr * W25Q_SECTOR_SIZE * 1024U * 16;
	if (size == 32)
		rawAddr /= 2;
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_ReadJEDECID(uint8_t *buf) {
	return W25Q_PARAM_ERR;
}
...
uint32_t page_to_addr(uint32_t pageNum, uint8_t pageShift) {
	return pageNum * W25Q_PAGE_SIZE + pageShift;
}
